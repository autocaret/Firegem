...
texteditor.addControl = function ( area )
{
	...
	obj.GenerateToolbar ( Array ( 
		...
		adv ? { 'type' : 'spacer' } : {},
		{ 'type' : 'button', 'classname' : 'bPurifyHTML', 'action' : 'purifyHTML ( )', 'title' : 'Rengjør utvalg', 'icon' : 'page_white_medal.png' },
	...
	obj.purifyHTML = function()
	{
		this.storeSelection ();
		
		let fhtml = this.getDocument().body.innerHTML;
		fhtml = fhtml.split( /\ style\=\".*?\"/i ).join( '' );
		
		// Fix document
		this.getDocument().body.innerHTML = fhtml;
					...
					else
					{
						...
						{	
							...
							for ( var a = 0; a < asr.length; a++ )
							{
								...
								pnodeWithoutTmpNode = pnodeWithoutTmpNode.split ( as.getAttribute ( 'href' ) ).join ( 'Firegemdelete' );
								pnodeWithoutTmpNode = pnodeWithoutTmpNode.split ( /\<a href\=\"Firegemdelete\"[^>]*?\>.*?\<\/a>/i ).join ( '' );
						...
						else
						{
							if ( as1 && as1.parentNode )
							{
								...
								if ( n.nodeName )
								{
									if ( as1.innerHTML.match ( /special_Firegem_link.*?done/i ) )
									{
										as1.innerHTML = as1.innerHTML.split ( /special_Firegem_link.*?done/i ).join ( '' );
									}
									as1.appendChild ( n );
	...
	{
		var linkc = 'special_Firegem_link' + ( Math.random ( ) * 9999 ) + ( Math.random ( ) * 9999 ) + '_done';
